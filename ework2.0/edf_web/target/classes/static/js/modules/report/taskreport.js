...
function searchreport(){
	taskPlannedhoursTotal=0;
	taskTotalhoursTotal=0;
	taskCyclehoursTotal=0;
	if (!$('#form').Validform()) {
        return false;
    }
	$.ajax({
...
function getCompany(){
	$.getJSON("../../api/sys/depart/listAll?_" + $.now(), function(data) {
		for ( var i in data) {
			$('#company').append('<option value='+data[i].depId+'> '+data[i].depName+'</option>')
		}
	});
}
