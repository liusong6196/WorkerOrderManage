	...
	$('#dataGrid').bootstrapTableExByBox({
		...
		columns: [
			...
			{field : "taskId",title : "实际工时", width : "20px",align:"center",formatter : function(value, row, index) {
				var actualhours=row.taskActualhours;
				if(null==actualhours){
					actualhours="";
				}
				return  "<div class='input-group input-group-sm' ><input type='text' class='form-control'  style='height:20px;text-align:center' id='actualhours"+value+"' value='"+actualhours+"'></div>"	;
			}},
			{field : "taskId", title : "操作", width : "100px",align:"center",formatter : function(value, row, index) {
				...
				return  "<a class='btn btn-success btn-xs' onclick=taskfinish('"+value+"','"+actualhours+"') ></i>完成任务</a>";
			}	}
		]
	})
}
...
function getGrid_ready() {
	$('#dataGrid_ready').bootstrapTableExByBox({
		...
		pageSize: 2,
		columns: [
			{field : "proNumber", title : "项目编号", width : "200px",align:"center"}, 
			...
			{field : "taskId", title : "操作", width : "100px",align:"center",formatter : function(value, row, index) {
				return  "<a class='btn btn-primary btn-xs' onclick=taskbegin('"+value+"') ></i>开始任务</a>";
				
				//return "<button type='button' class='btn btn-block btn-success btn-xs'>Success</button>";
			}	}
		]
	})
}
...
function getGrid_question() {
	$('#dataGrid_question').bootstrapTableExByBox({
		...
		columns: [
			...
			{field : "proQueStatus", title : "状态", width : "50px",align:"center",formatter : function(value, row, index) {
				switch (value){
				case  "0": return "待解决";
				case  "1": return "解决中";
				case  "2": return "已解决";
			...
			{field : "gmtCreate", title : "创建时间", width : "100px",align:"center",formatter : function(value, row, index) {
				return value.substr(0,11);
			}}, 
			{field : "gmtModified", title : "修改时间", width : "100px",align:"center",formatter : function(value, row, index) {
				return value.substr(0,11);
			}}
		]
	})
...
$.fn.bootstrapTableExByBox = function(opt){
	var defaults = {
		...
		rowStyle: function (row, index) {
            var style = {};             
                style={css:{'text-align':'center','vertical-align':'middle'}};                
            return style;
        }    
	}
...
function taskfinish(v,actualhours){
	...
	dialogConfirm("确定已完成任务？",function(){
		if(""!=actualhours){
			dialogConfirmtimesheet("timesheet已经填写过工时，是否覆盖？",function(){
			$.ajax({
				type: "post",
				url : '../../api/tasklist/updatestatus?_' + $.now(),
				data:{
			...
			$.ajax({
				type: "post",
				url : '../../api/tasklist/updatestatusNotTimeSheet?_' + $.now(),
				data:{
					taskId:v,
					status:"4",
					date:formatDate(new Date(),"yyyy-MM-dd"),
